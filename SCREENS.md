...
| `/crm/google-ads` | `crm-admin/google-ads/GoogleAdsAdmin.tsx` | **Dashboard Google Ads (Sprint 2 — read-only).** Snapshot dos últimos 30 dias (campanhas, ad groups, keywords, asset groups) a partir de `crm.google_*`. Cards KPI (Gasto, Impressões, Cliques, CTR, CPC, Conversões, Valor conv.), tabela de campanhas com drill-down ad group → keyword, badge de status, indicador de última sincronização e botão "Sincronizar agora" que invoca `crm-google-ads-sync`. Toggle no topo alterna com o Dashboard Meta Live (`/crm`). RBAC: admin / marketing_manager / platform_admin. Restantes tabs (Conversões offline / Audiences / Definições) ficam placeholder até à Data Manager API. |
