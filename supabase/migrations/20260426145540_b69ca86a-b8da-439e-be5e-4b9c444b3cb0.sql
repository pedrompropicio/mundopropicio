...
BEGIN
  ...
  -- Opção C: snapshot da formalidade da Ativa atual (por categoria+descrição) ANTES de apagar
  ...
  SELECT
    -- Reutiliza id antigo se houver match (preserva log via FK CASCADE), senão gera novo
    ...
    r->>'specification',
    v_now, v_now, NULL,
    -- Merge de formalidade: usa a da Ativa anterior; se for nova linha, default 'estimado'
  ...
  LOOP
    ...
    -- Opção C para o Split: preservar formalidade da Ativa do Split
