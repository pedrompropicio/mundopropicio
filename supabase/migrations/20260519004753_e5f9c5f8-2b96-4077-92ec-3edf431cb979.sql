-- BUCKET
INSERT INTO storage.buckets (id, name, public) VALUES ('operacao-media','operacao-media',false) ON CONFLICT (id) DO NOTHING;
...
CREATE TABLE IF NOT EXISTS public.operacao_frentes (
  ...
  color text DEFAULT '#3b82f6', display_order int NOT NULL DEFAULT 0,
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_op_team_permanent_lead ON public.operacao_frente_team(frente_id) WHERE role_in_frente='lead' AND is_permanent_lead=true;
...
CREATE TABLE IF NOT EXISTS public.operacao_etapas (
  ...
  company_id uuid NOT NULL DEFAULT public.current_company_id(),
  frente_id uuid NOT NULL REFERENCES public.operacao_frentes(id) ON DELETE CASCADE,
  name text NOT NULL, escopo text,
...
CREATE TABLE IF NOT EXISTS public.operacao_registros (
  ...
  text text, audio_url text, transcribed_text text,
  priority text CHECK (priority IN ('crit','high','med','low')),
  status text CHECK (status IN ('open','in_progress','resolved','closed')),
  acked_at timestamptz, acked_by_profile_id uuid REFERENCES public.profiles(id),
  resolved_at timestamptz, resolved_by_profile_id uuid REFERENCES public.profiles(id),
  sla_due_at timestamptz, sla_half_at timestamptz, escalation_level int NOT NULL DEFAULT 0,
  metadata jsonb NOT NULL DEFAULT '{}'::jsonb,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT chamado_requires_priority_status CHECK (kind <> 'chamado' OR (priority IS NOT NULL AND status IS NOT NULL)),
  CONSTRAINT status_only_for_chamado CHECK (status IS NULL OR kind = 'chamado')
);
...
CREATE INDEX IF NOT EXISTS idx_op_reg_chamado_sla ON public.operacao_registros(kind, status, sla_due_at) WHERE kind='chamado' AND status IN ('open','in_progress');
...
CREATE TABLE IF NOT EXISTS public.operacao_mentions (
  ...
  notified_at timestamptz, read_at timestamptz,
  created_at timestamptz NOT NULL DEFAULT now()
);
...
INSERT INTO public.operacao_chamado_sla(priority, sla_minutes) VALUES ('crit',15),('high',60),('med',240),('low',1440) ON CONFLICT (priority) DO NOTHING;
...
CREATE TABLE IF NOT EXISTS public.operacao_daily_reports (
  ...
  pdf_url text, summary_json jsonb NOT NULL DEFAULT '{}'::jsonb,
...
-- updated_at triggers
DO $$ DECLARE t text;
BEGIN FOREACH t IN ARRAY ARRAY['operacao_frentes','operacao_etapas','operacao_registros'] LOOP
...
-- RLS
DO $$ DECLARE t text;
BEGIN FOREACH t IN ARRAY ARRAY['operacao_frentes','operacao_frente_team','operacao_etapas','operacao_registros','operacao_registro_media','operacao_mentions','operacao_chamado_sla','operacao_daily_reports'] LOOP
...
DO $$ DECLARE t text;
BEGIN FOREACH t IN ARRAY ARRAY['operacao_frentes','operacao_frente_team','operacao_etapas','operacao_registros','operacao_registro_media','operacao_mentions','operacao_daily_reports'] LOOP
...
CREATE POLICY "op_frentes_ins" ON public.operacao_frentes FOR INSERT TO authenticated WITH CHECK (public.has_permission(auth.uid(),'manage_operacao_frentes'));
DROP POLICY IF EXISTS "op_frentes_upd" ON public.operacao_frentes;
CREATE POLICY "op_frentes_upd" ON public.operacao_frentes FOR UPDATE TO authenticated USING (public.has_permission(auth.uid(),'manage_operacao_frentes')) WITH CHECK (public.has_permission(auth.uid(),'manage_operacao_frentes'));
DROP POLICY IF EXISTS "op_frentes_del" ON public.operacao_frentes;
CREATE POLICY "op_frentes_del" ON public.operacao_frentes FOR DELETE TO authenticated USING (public.has_role(auth.uid(),'admin'::app_role) OR public.has_role(auth.uid(),'manager'::app_role));

DROP POLICY IF EXISTS "op_team_ins" ON public.operacao_frente_team;
CREATE POLICY "op_team_ins" ON public.operacao_frente_team FOR INSERT TO authenticated WITH CHECK (public.has_permission(auth.uid(),'manage_operacao_frentes'));
DROP POLICY IF EXISTS "op_team_upd" ON public.operacao_frente_team;
CREATE POLICY "op_team_upd" ON public.operacao_frente_team FOR UPDATE TO authenticated USING (public.has_permission(auth.uid(),'manage_operacao_frentes')) WITH CHECK (public.has_permission(auth.uid(),'manage_operacao_frentes'));
DROP POLICY IF EXISTS "op_team_del" ON public.operacao_frente_team;
CREATE POLICY "op_team_del" ON public.operacao_frente_team FOR DELETE TO authenticated USING (public.has_role(auth.uid(),'admin'::app_role) OR public.has_role(auth.uid(),'manager'::app_role));

DROP POLICY IF EXISTS "op_etapas_ins" ON public.operacao_etapas;
CREATE POLICY "op_etapas_ins" ON public.operacao_etapas FOR INSERT TO authenticated WITH CHECK (public.has_permission(auth.uid(),'manage_operacao_etapas') OR EXISTS (SELECT 1 FROM public.operacao_frentes f WHERE f.id = frente_id AND f.current_lead_id = auth.uid()));
DROP POLICY IF EXISTS "op_etapas_upd" ON public.operacao_etapas;
CREATE POLICY "op_etapas_upd" ON public.operacao_etapas FOR UPDATE TO authenticated USING (public.has_permission(auth.uid(),'manage_operacao_etapas') OR EXISTS (SELECT 1 FROM public.operacao_frentes f WHERE f.id = frente_id AND f.current_lead_id = auth.uid())) WITH CHECK (public.has_permission(auth.uid(),'manage_operacao_etapas') OR EXISTS (SELECT 1 FROM public.operacao_frentes f WHERE f.id = frente_id AND f.current_lead_id = auth.uid()));
DROP POLICY IF EXISTS "op_etapas_del" ON public.operacao_etapas;
CREATE POLICY "op_etapas_del" ON public.operacao_etapas FOR DELETE TO authenticated USING (public.has_role(auth.uid(),'admin'::app_role) OR public.has_role(auth.uid(),'manager'::app_role));

DROP POLICY IF EXISTS "op_reg_ins_chamado" ON public.operacao_registros;
CREATE POLICY "op_reg_ins_chamado" ON public.operacao_registros FOR INSERT TO authenticated WITH CHECK (kind='chamado' AND public.has_permission(auth.uid(),'open_chamado'));
DROP POLICY IF EXISTS "op_reg_ins_other" ON public.operacao_registros;
CREATE POLICY "op_reg_ins_other" ON public.operacao_registros FOR INSERT TO authenticated WITH CHECK (kind <> 'chamado' AND public.has_permission(auth.uid(),'register_operacao'));
DROP POLICY IF EXISTS "op_reg_upd" ON public.operacao_registros;
CREATE POLICY "op_reg_upd" ON public.operacao_registros FOR UPDATE TO authenticated USING (public.has_permission(auth.uid(),'manage_chamados') OR public.has_permission(auth.uid(),'register_operacao')) WITH CHECK (public.has_permission(auth.uid(),'manage_chamados') OR public.has_permission(auth.uid(),'register_operacao'));
DROP POLICY IF EXISTS "op_reg_del" ON public.operacao_registros;
CREATE POLICY "op_reg_del" ON public.operacao_registros FOR DELETE TO authenticated USING (public.has_role(auth.uid(),'admin'::app_role) OR public.has_role(auth.uid(),'manager'::app_role));

DO $$ DECLARE t text;
BEGIN FOREACH t IN ARRAY ARRAY['operacao_registro_media','operacao_mentions'] LOOP
...
CREATE POLICY "op_daily_ins" ON public.operacao_daily_reports FOR INSERT TO authenticated WITH CHECK (public.has_permission(auth.uid(),'manage_operacao_frentes'));
DROP POLICY IF EXISTS "op_daily_upd" ON public.operacao_daily_reports;
CREATE POLICY "op_daily_upd" ON public.operacao_daily_reports FOR UPDATE TO authenticated USING (public.has_permission(auth.uid(),'manage_operacao_frentes')) WITH CHECK (public.has_permission(auth.uid(),'manage_operacao_frentes'));
DROP POLICY IF EXISTS "op_daily_del" ON public.operacao_daily_reports;
CREATE POLICY "op_daily_del" ON public.operacao_daily_reports FOR DELETE TO authenticated USING (public.has_role(auth.uid(),'admin'::app_role) OR public.has_role(auth.uid(),'manager'::app_role));

DROP POLICY IF EXISTS "operacao_media_select" ON storage.objects;
CREATE POLICY "operacao_media_select" ON storage.objects FOR SELECT TO authenticated USING (bucket_id='operacao-media' AND public.has_permission(auth.uid(),'view_operacao'));
DROP POLICY IF EXISTS "operacao_media_insert" ON storage.objects;
CREATE POLICY "operacao_media_insert" ON storage.objects FOR INSERT TO authenticated WITH CHECK (bucket_id='operacao-media' AND public.has_permission(auth.uid(),'register_operacao'));
DROP POLICY IF EXISTS "operacao_media_update" ON storage.objects;
CREATE POLICY "operacao_media_update" ON storage.objects FOR UPDATE TO authenticated USING (bucket_id='operacao-media' AND public.has_permission(auth.uid(),'register_operacao'));
DROP POLICY IF EXISTS "operacao_media_delete" ON storage.objects;
CREATE POLICY "operacao_media_delete" ON storage.objects FOR DELETE TO authenticated USING (bucket_id='operacao-media' AND (public.has_role(auth.uid(),'admin'::app_role) OR public.has_role(auth.uid(),'manager'::app_role)));
...
CREATE TRIGGER trg_operacao_set_sla BEFORE INSERT ON public.operacao_registros FOR EACH ROW EXECUTE FUNCTION public.trg_operacao_set_sla();
...
BEGIN
  IF NEW.is_permanent_lead=true AND NEW.role_in_frente='lead' THEN
    UPDATE public.operacao_frentes SET current_lead_id=NEW.profile_id WHERE id=NEW.frente_id AND lead_handover_until IS NULL;
  END IF;
  RETURN NEW;
END $$;
DROP TRIGGER IF EXISTS trg_op_team_lead_sync ON public.operacao_frente_team;
CREATE TRIGGER trg_op_team_lead_sync AFTER INSERT OR UPDATE ON public.operacao_frente_team FOR EACH ROW EXECUTE FUNCTION public.trg_operacao_frente_lead_sync();

DO $$ DECLARE t text;
BEGIN FOREACH t IN ARRAY ARRAY['operacao_frentes','operacao_frente_team','operacao_etapas','operacao_registros'] LOOP
...
SELECT cron.schedule('operacao-handover-restore','*/1 * * * *', $cron$
  UPDATE public.operacao_frentes f
     SET current_lead_id = (SELECT t.profile_id FROM public.operacao_frente_team t WHERE t.frente_id=f.id AND t.role_in_frente='lead' AND t.is_permanent_lead=true LIMIT 1),
