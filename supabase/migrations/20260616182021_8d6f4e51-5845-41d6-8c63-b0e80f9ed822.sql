...
-- ============================================================================
-- Fase 2b — Partner pode CRIAR e APAGAR linhas no BP (direto na grelha)
-- ============================================================================

-- 1) Policy INSERT partner em event_forecasts
DROP POLICY IF EXISTS event_forecasts_insert_partner ON public.event_forecasts;
CREATE POLICY event_forecasts_insert_partner ON public.event_forecasts
  ...
  WITH CHECK (
    ...
    AND type = 'expense'
    AND COALESCE(is_overhead, false) = false
    AND COALESCE(exclude_from_result, false) = false
    AND master_forecast_id IS NULL
    AND COALESCE(is_retroactive_override, false) = false
  );

-- 2) Policy DELETE partner em event_forecasts
DROP POLICY IF EXISTS event_forecasts_delete_partner ON public.event_forecasts;
CREATE POLICY event_forecasts_delete_partner ON public.event_forecasts
...
-- 3) RPC batch_insert_event_forecasts: aceitar partner com can_edit_bp + edit_approved_bp
--    (força type='expense' quando o caller é partner)
...
BEGIN
  ...
  FOR v_ins IN SELECT * FROM jsonb_array_elements(_inserts) LOOP
    ...
    -- Partner só pode criar despesas
    ...
    INSERT INTO event_forecasts (
      event_id, type, description, amount, iva_rate, category_id,
