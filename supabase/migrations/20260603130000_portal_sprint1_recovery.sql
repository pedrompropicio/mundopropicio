...
--
-- Restaura o schema do Sprint 1 (aplicado 28/05/2026 via SQL Editor directo,
-- sem tracking em supabase_migrations.schema_migrations) que desapareceu de
-- Live em 03/06/2026 (causa provável: restore de backup automático OU
-- propagação Test→Live num Publish sem este estado nunca ter sido tracked).
--
-- Spec: docs/portal/data-model.md (§2.1-§2.7, §4)
--       docs/portal/mvp-spec.md (L18 — slug canónico "anitta-eda-2026")
--
-- Tudo idempotente: ADD COLUMN IF NOT EXISTS, CREATE TABLE IF NOT EXISTS,
-- DROP POLICY IF EXISTS + CREATE, CREATE OR REPLACE VIEW, UPDATE WHERE id.
--
-- Origem das partes:
--   [DOC §X.Y] — vem directamente da spec data-model.md
--   [INF]      — inferência (padrão ERP, requisitos Postgres RLS)
--   [PEDRO]    — confirmado pelo Pedro fora da doc (transcript)

BEGIN;

-- ─── Extensão pgcrypto [DOC §2.4 — generated columns precisam digest()] ──
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- ─── 1. public.events: 14 colunas portal ──────────────────────────────
-- [DOC §2.1 L96-108] — 13 colunas
...
-- ─── 6. lead_capture (proxy INSERT anónimo) [DOC §2.7 L315-347] ──────
...
-- ─── 7. redirect_log (proxy INSERT anónimo) [DOC §2.7 L350-369] ──────
...
-- ─── 8. RLS policies ──────────────────────────────────────────────────
-- [INF — padrão ERP 20260317112302 (PERMISSIVE base FOR ALL TO authenticated)
--  + 20260429022451 (RESTRICTIVE company_isolation_*). Postgres exige
--  PERMISSIVE para acesso; RESTRICTIVE para scoping.]
...
-- contacts: SELECT só para admin/editor [DOC §4 L479]
-- Helper confirmado: public.has_role(uuid, app_role) — definida em
-- 20260515013743. Restringe a admin/editor da empresa.
...
-- leads: SELECT só para admin/editor [DOC §4 L479]
...
-- lead_capture: anon INSERT only [DOC §2.7 L342-343 — SQL exacto da doc]
...
-- redirect_log: anon INSERT only [DOC §2.7 L368-369 — análogo]
...
-- events_public [DOC §2.6 L282-306 — SQL da doc + meta_pixel_id]
...
-- event_lineups_public [DOC §2.6 L307 "análoga" — INF SQL]
...
-- event_faqs_public [DOC §2.6 L307 "análoga" — INF SQL]
...
-- ─── 10. Grants ───────────────────────────────────────────────────────
-- [DOC §2.6 L305] anon, authenticated leem views públicas
...
-- [INF — Postgres exige GRANT além de POLICY para anon escrever]
GRANT INSERT ON public.lead_capture TO anon, authenticated;
GRANT INSERT ON public.redirect_log TO anon, authenticated;

-- (GRANT ALL TO service_role REMOVIDO — doc §2.7 L346 diz que é default.)

-- ─── 11. Backfill Ivete + Anitta [PEDRO — transcript confirmado] ─────
-- Slug Anitta confirmado em mvp-spec.md L18 ("anitta-eda-2026").
-- Slug Ivete consistente com URL Ticketline "ivete-clareou-portugal".
-- COALESCE preserva valor existente se já backfilled (idempotente).
