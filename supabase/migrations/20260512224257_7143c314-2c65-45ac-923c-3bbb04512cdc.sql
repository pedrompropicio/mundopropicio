...
BEGIN
  ...
  campaign_norm AS (
    -- Remove bracket prefixes ([SoldOut] [Vendas] [F] [Q] [Tráfego] [LS] [Alcance] etc.)
    -- antes de tokenizar para não inflacionar o score com palavras genéricas dos brackets
    SELECT
  ...
  campaign_matches AS (
    SELECT
      ...
      (
        SELECT count(*)::int
        FROM regexp_split_to_table(et.normalized, ' ') AS tok
        WHERE length(tok) >= 4
          AND cn.clean_name LIKE '%' || tok || '%'
      ) AS score,
      et.event_date,
      row_number() OVER (
