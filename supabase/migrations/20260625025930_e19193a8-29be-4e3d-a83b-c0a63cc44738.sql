...
-- PARTE 1: marcar todos os leads antigos (>7d) como skipped_old num único UPDATE
UPDATE public.leads
SET capi_sent_at = now(), capi_status = 'skipped_old'
WHERE capi_sent_at IS NULL
  AND created_at < now() - interval '7 days';

-- PARTE 2: RPC igual à anterior, só muda ORDER BY ASC -> DESC
