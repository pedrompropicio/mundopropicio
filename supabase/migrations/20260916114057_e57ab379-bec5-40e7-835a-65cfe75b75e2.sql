...
BEGIN
  ...
  IF v_check_bp_line THEN
    ...
    --
    -- Quinta isencao (16/09/2026, D-ERP69): shared_cost_account_id preenchido —
    -- linha de custo partilhado com terceiros. Nao consome verba do BP por
    -- definicao (e dinheiro de terceiros) e o trigger que lhe poe
    -- exclude_from_result corre DEPOIS deste, logo a marca ainda nao esta
    -- visivel aqui. Pertence a mesma lista.
    IF NEW.type = 'expense'
...
REVOKE EXECUTE ON FUNCTION public.enforce_transaction_approval_permission() FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.enforce_transaction_approval_permission() FROM anon;
REVOKE EXECUTE ON FUNCTION public.enforce_transaction_approval_permission() FROM authenticated;
GRANT EXECUTE ON FUNCTION public.enforce_transaction_approval_permission() TO service_role;