...
CREATE POLICY "Users see their own company" ON public.companies FOR SELECT TO authenticated
...
CREATE POLICY "Platform admin manages companies" ON public.companies FOR ALL TO authenticated
  USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());

DROP POLICY IF EXISTS "Read invitations" ON public.company_invitations;
CREATE POLICY "Read invitations" ON public.company_invitations FOR SELECT TO authenticated
  USING (public.is_platform_admin() OR (company_id = public.current_company_id() AND public.has_role(auth.uid(), 'admin'::app_role)));

DROP POLICY IF EXISTS "Write invitations" ON public.company_invitations;
CREATE POLICY "Write invitations" ON public.company_invitations FOR ALL TO authenticated
  USING (public.is_platform_admin() OR (company_id = public.current_company_id() AND public.has_role(auth.uid(), 'admin'::app_role)))
  WITH CHECK (public.is_platform_admin() OR (company_id = public.current_company_id() AND public.has_role(auth.uid(), 'admin'::app_role)));
...
CREATE POLICY "Public read company branding" ON storage.objects FOR SELECT
  USING (bucket_id = 'company-branding');

DROP POLICY IF EXISTS "Platform admin manages company branding" ON storage.objects;
CREATE POLICY "Platform admin manages company branding" ON storage.objects FOR ALL TO authenticated
...
VALUES ('Mundo Propício, Lda', 'Mundo Propício', 'mundo-propicio', 'PT', 'EUR', 'Europe/Lisbon', 'adm@mundopropicio.com')
ON CONFLICT (slug) DO NOTHING;

UPDATE public.profiles SET company_id = (SELECT id FROM public.companies WHERE slug = 'mundo-propicio') WHERE company_id IS NULL;
UPDATE public.user_roles SET company_id = (SELECT id FROM public.companies WHERE slug = 'mundo-propicio') WHERE company_id IS NULL;
UPDATE public.user_permissions SET company_id = (SELECT id FROM public.companies WHERE slug = 'mundo-propicio') WHERE company_id IS NULL;

INSERT INTO public.user_roles (user_id, role, company_id)
SELECT id, 'platform_admin'::app_role, NULL
  FROM public.profiles
 WHERE email = 'pedroneto@mundopropicio.com'
   AND NOT EXISTS (SELECT 1 FROM public.user_roles WHERE user_id = profiles.id AND role = 'platform_admin'::app_role);