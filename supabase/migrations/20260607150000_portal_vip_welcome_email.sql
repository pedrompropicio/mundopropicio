...
--
-- 1) Adiciona setting 'general.vip_coupon_code' em portal_settings (seed
--    idempotente). Editável via /crm/portal-settings sem alterações ao React.
--
-- 2) Trigger AFTER INSERT em public.lead_capture que enfileira um email
--    de boas-vindas VIP para a fila pgmq 'transactional_emails' (mesmo
--    schema que send-transactional-email enfileira; consumido por
--    process-email-queue → sendLovableEmail). Localizado PT/EN via
--    NEW.raw->>'locale'. Inclui código de cupão da setting; se vazio,
--    versão graciosa sem código. Se event_slug presente, referencia o
--    evento; senão, mensagem genérica.
--
-- Idempotência: 1 dispatch por INSERT (trigger row-level). idempotency_key
-- por lead_capture.id permite supressão downstream se algo re-enfileirar.
--
-- NÃO altera caminho de inserção, NÃO toca em client_event_id nem na
-- lógica de pixel/CAPI. EXCEPTION handler garante que falha de
