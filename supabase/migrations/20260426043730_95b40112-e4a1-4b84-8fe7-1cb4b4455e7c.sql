...
ALTER TABLE public.event_sessions
  ADD COLUMN version_id uuid REFERENCES public.bp_versions(id) ON DELETE CASCADE;
...
CREATE INDEX idx_event_sessions_event_version ON public.event_sessions(event_id, version_id);
CREATE INDEX idx_event_ticket_zones_event_version ON public.event_ticket_zones(event_id, version_id);
CREATE INDEX idx_event_ticket_lots_zone_version ON public.event_ticket_lots(zone_id, version_id);
...
BEGIN
  ...
  -- ─── Clone ticketing structure for scenario drafts (Master) ───
  IF _scenario_label IS NOT NULL THEN
    v_session_map := '{}'::jsonb;
    v_zone_map := '{}'::jsonb;

    -- Clone sessions (only Active rows: version_id IS NULL)
  ...
  IF v_is_master THEN
    FOR v_split IN SELECT id, name, status FROM public.events WHERE parent_event_id = _event_id LOOP
      ...
      -- Clone ticketing for scenario splits
      IF _scenario_label IS NOT NULL THEN
        v_session_map := '{}'::jsonb;
        v_zone_map := '{}'::jsonb;
...
DECLARE
  ...
  -- ticketing remap maps for promotion
  v_session_map jsonb;
  v_zone_map jsonb;
  v_old_id uuid; v_new_id uuid;
  v_session_row RECORD; v_zone_row RECORD; v_lot_row RECORD;
BEGIN
  ...
  -- Rewrite event_forecasts (Active rows only) from scenario rows.
  DELETE FROM public.event_forecasts WHERE event_id = v_scenario.event_id AND version_id IS NULL;

  -- Promote scenario forecast rows: copy them to Active by clearing version_id and reusing snapshot_payload.
  ...
  -- ─── Promote ticketing structure: replace Active with scenario rows ───
  -- Strategy: delete current Active rows, then re-clone from scenario rows (set version_id=NULL).
  ...
  -- Apply automatic bypass reconciliation.
  IF NOT _force THEN
    BEGIN
  ...
  -- Other scenarios actions (keep / archive / discard)
  ...
  -- Cascade Master → Splits (forecasts + ticketing)
  FOR v_split IN
  ...
  LOOP
    ...
    -- Promote ticketing for the split
    DELETE FROM public.event_ticket_lots l
     USING public.event_ticket_zones z
