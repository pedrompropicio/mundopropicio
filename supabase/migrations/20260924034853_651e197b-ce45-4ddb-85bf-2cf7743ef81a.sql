CREATE OR REPLACE VIEW public.v_song_benchmark_aligned WITH (security_invoker = true) AS  WITH launch AS (
 ...
 SELECT p.launch_song_id,
    ...
    tt.source AS tiktok_ugc_source,
        CASE
            WHEN tt.value IS NOT NULL AND tt.metric_date IS NOT NULL THEN round(tt.value / GREATEST(tt.metric_date - s.release_date, 1)::numeric, 2)
            ELSE NULL::numeric
   ...
   FROM pairs p
     ...
     LEFT JOIN LATERAL ( SELECT m.value,
            ...
            m.source
           FROM artist_song_metrics_daily m
          WHERE m.song_id = p.song_id AND m.platform = 'tiktok'::text AND m.metric = 'ugc_videos'::text
            AND (m.source = ANY (ARRAY['ios_shortcut'::text, 'manual'::text]))
          ORDER BY m.metric_date DESC, (m.source = 'ios_shortcut'::text) DESC
         LIMIT 1) tt ON true
     LEFT JOIN LATERAL ( SELECT m.value,
            m.metric_date
           FROM artist_song_metrics_daily m
