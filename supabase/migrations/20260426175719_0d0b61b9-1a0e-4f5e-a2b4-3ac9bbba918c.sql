DROP FUNCTION IF EXISTS public._debug_analyze_formalidade(uuid[]);

CREATE OR REPLACE FUNCTION public.analyze_formalidade_bulk(_event_ids uuid[] DEFAULT NULL::uuid[])
...
BEGIN
  ...
  direct_tx AS (
    -- Apenas linhas COM transaction_id direto.
    SELECT
      af.id AS fid,
    ...
    JOIN public.transactions t ON t.id = af.transaction_id
    WHERE af.transaction_id IS NOT NULL
    GROUP BY af.id
  ),
  category_tx AS (
    SELECT
  ...
  SELECT
    ...
    CASE
      ...
      WHEN c.has_category_match
           AND c.bp_amount > 0
           AND c.paid_total > 0
           AND ABS(c.paid_total - c.bp_amount) / c.bp_amount <= v_tolerance
        THEN 'high'
      WHEN c.has_category_match
           AND c.bp_amount > 0
           AND c.paid_total = 0
           AND c.approved_total > 0
           AND ABS(c.approved_total - c.bp_amount) / c.bp_amount <= v_tolerance
        THEN 'high'
      ELSE 'low'
    END AS confidence,
    CASE
      ...
      WHEN c.has_category_match
           AND c.bp_amount > 0
           AND c.paid_total > 0
           AND ABS(c.paid_total - c.bp_amount) / c.bp_amount <= v_tolerance THEN
        format('Match por categoria — %s TX paga(s) totalizando %s€ ≈ BP %s€ (±5%%)',
