...
BEGIN
  ...
  -- Guarda: só dispara para leads da MP
  IF NEW.company_id IS DISTINCT FROM MP_COMPANY_ID THEN
    RETURN NEW;
  END IF;

  -- Salvaguarda extra: sources claramente não-MP (ex.: Coala) nunca recebem VIP MP
  IF NEW.source IS NOT NULL AND NEW.source LIKE 'coala%' THEN
    RETURN NEW;
  END IF;

  -- Supressão: evento com cupom próprio ativo -> vip-coupon-email cobre tudo
  IF NEW.source IS NOT NULL AND NEW.source LIKE 'vip%'
     AND NEW.event_slug IS NOT NULL AND NEW.event_slug <> '' THEN
    SELECT true INTO v_event_coupon
    FROM public.events e
    WHERE e.company_id = MP_COMPANY_ID
      AND e.slug = NEW.event_slug
      AND NULLIF(btrim(e.vip_coupon_code), '') IS NOT NULL
      AND e.vip_coupon_valid_until >= current_date
    LIMIT 1;

    IF v_event_coupon IS TRUE THEN
      RETURN NEW;
    END IF;
  END IF;
  ...
  FROM public.portal_settings
  WHERE company_id = MP_COMPANY_ID
    AND key = 'general.vip_coupon_discount_label'
  LIMIT 1;
  v_label := COALESCE(v_label,
    CASE WHEN v_lang = 'en' THEN 'VIP discount' ELSE 'desconto VIP' END);
  ...
  IF v_lang = 'en' THEN
    ...
    IF v_coupon IS NOT NULL THEN
      v_subject := 'Welcome to VIP — your ' || v_label || ' code';
    ELSE
    ...
    v_text :=
      ...
      CASE WHEN v_coupon IS NOT NULL THEN
        'You now get ' || v_label || ' across all events on sale.' || E'\n\n' ||
        'Your VIP code: ' || v_coupon || E'\n' ||
        'Use it at checkout on the ticketing site.' || E'\n\n'
      ELSE
        'You''ll be the first to know about presales and VIP-only news.' || E'\n\n'
      END ||
      ...
      '<div style="font-family:Helvetica,Arial,sans-serif;background:#0a0a0a;padding:32px 0;color:#e9e6dc">' ||
        '<div style="max-width:560px;margin:0 auto;padding:0 24px">' ||
          ...
          CASE WHEN v_event_name IS NOT NULL
               ...
               ELSE '' END ||
          CASE WHEN v_coupon IS NOT NULL THEN
            '<p style="font-size:15px;line-height:1.6;margin:0 0 20px 0">You now get <strong>' || v_label || '</strong> across all events on sale.</p>' ||
            ...
            '</div>'
          ELSE
            '<p style="font-size:15px;line-height:1.6;margin:0 0 20px 0">You''ll be the first to know about presales and VIP-only news.</p>'
          END ||
  ...
  ELSE
    ...
    IF v_coupon IS NOT NULL THEN
      v_subject := 'Bem-vindo ao VIP — o teu código de ' || v_label;
    ELSE
    ...
    v_text :=
      ...
      CASE WHEN v_coupon IS NOT NULL THEN
        'A partir de agora tens ' || v_label || ' em todos os eventos à venda.' || E'\n\n' ||
        'O teu código VIP: ' || v_coupon || E'\n' ||
        'Aplica-o no checkout da bilheteira.' || E'\n\n'
      ELSE
        'Vais saber em primeira mão das pré-vendas e das novidades só para VIP.' || E'\n\n'
      END ||
      ...
      '<div style="font-family:Helvetica,Arial,sans-serif;background:#0a0a0a;padding:32px 0;color:#e9e6dc">' ||
        '<div style="max-width:560px;margin:0 auto;padding:0 24px">' ||
          ...
          CASE WHEN v_event_name IS NOT NULL
               ...
               ELSE '' END ||
          CASE WHEN v_coupon IS NOT NULL THEN
            '<p style="font-size:15px;line-height:1.6;margin:0 0 20px 0">A partir de agora tens <strong>' || v_label || '</strong> em todos os eventos à venda.</p>' ||
            '<div style="background:linear-gradient(135deg,#1a1a1a,#2a2410);border:1px solid #d4af37;border-radius:8px;padding:20px;text-align:center;margin:24px 0">' ||
            ...
            '</div>'
          ELSE
            '<p style="font-size:15px;line-height:1.6;margin:0 0 20px 0">Vais saber em primeira mão das pré-vendas e das novidades só para VIP.</p>'
          END ||
          '<div style="text-align:center;margin:28px 0 8px 0">' ||
