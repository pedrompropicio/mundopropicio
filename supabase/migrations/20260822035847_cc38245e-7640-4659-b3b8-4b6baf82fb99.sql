...
BEGIN
  ...
  -- TESTE 1: vínculo cruzado recusado
  BEGIN
  ...
  -- TESTE 2: tx.event_id IS NULL continua a passar
  BEGIN
  ...
  -- TESTE 3: auto-desvínculo ao mudar de evento
  ...
  -- TESTE 5: sync de rubrica no caso normal (mesmo evento)
  SELECT f.id, f.transaction_id, f.category_id INTO v_fc_linked, v_tx_linked, v_old_cat
    FROM event_forecasts f JOIN transactions t ON t.id = f.transaction_id
   WHERE f.event_id = v_anitta AND f.version_id IS NULL AND t.event_id = v_anitta
     AND f.category_id IS NOT NULL
   LIMIT 1;
  SELECT id INTO v_new_cat FROM account_categories
   WHERE id <> v_old_cat AND level = 3 AND type = (SELECT type FROM account_categories WHERE id = v_old_cat)
     AND company_id = (SELECT company_id FROM account_categories WHERE id = v_old_cat)
   LIMIT 1;
  BEGIN
...
REVOKE ALL ON FUNCTION public._test_bp_tx_guards() FROM PUBLIC;