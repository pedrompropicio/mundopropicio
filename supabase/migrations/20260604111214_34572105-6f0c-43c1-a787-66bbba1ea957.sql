-- =========================================================================
-- Portal MP — RPCs SECURITY DEFINER para bypass de RLS nas edge functions
-- (workaround para SUPABASE_SERVICE_ROLE_KEY mal-mapeada em Lovable Cloud)
-- =========================================================================

-- -------------------------------------------------------------------------
-- 1) process_lead_captures_batch
-- -------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.process_lead_captures_batch(p_batch_size int DEFAULT 50)
RETURNS SETOF jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- match contact: email -> phone_e164
      v_contact := NULL;
      ...
      IF v_contact.id IS NOT NULL THEN
        UPDATE public.contacts SET
          ...
          email_hash_sha256 = coalesce(email_hash_sha256, v_email_hash),
          phone_hash_sha256 = coalesce(phone_hash_sha256, v_phone_hash),
      ...
      ELSE
        INSERT INTO public.contacts (
          ...
          email_hash_sha256, phone_hash_sha256,
      ...
      -- montar payload CAPI (ou skip se sem pixel)
...
REVOKE ALL ON FUNCTION public.process_lead_captures_batch(int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.process_lead_captures_batch(int) TO authenticated, anon, service_role;

-- -------------------------------------------------------------------------
-- 2) process_redirect_logs_batch
-- -------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.process_redirect_logs_batch(p_batch_size int DEFAULT 50)
RETURNS SETOF jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, extensions, pg_temp
...
REVOKE ALL ON FUNCTION public.process_redirect_logs_batch(int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.process_redirect_logs_batch(int) TO authenticated, anon, service_role;
