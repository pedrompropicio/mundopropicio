...
-- Novo overload com p_release_for_repayment. Mantém o overload 4-arg antigo por compatibilidade
-- (chama o novo com release=false).
...
BEGIN
  ...
  -- Remoção de listas de pagamento
  -- draft/pending_approval: SEMPRE removidas (as duas modalidades)
  ...
  -- approved/paid: só removidas quando release=true; caso contrário ficam com aviso na lista
  IF p_release_for_repayment THEN
    FOR v_list IN
      SELECT pl.id AS list_id, pl.name AS list_name, pl.status AS list_status, pli.id AS pli_id
    ...
    END LOOP;
  ELSE
    -- Aviso no histórico da transação para cada lista aprovada/paga que ainda a contém
    FOR v_list IN
      SELECT pl.id AS list_id, pl.name AS list_name, pl.status AS list_status
        FROM public.payment_list_items pli
        JOIN public.payment_lists pl ON pl.id = pli.payment_list_id
       WHERE pli.transaction_id = p_tx_id
         AND pl.status IN ('approved','paid')
...
-- Overload 4-arg: encaminha para o novo com release=false (mantém compatibilidade)
CREATE OR REPLACE FUNCTION public.reverse_transaction(
  p_tx_id uuid,
  p_kind text,
  p_reason text,
...
SET search_path TO 'public'
AS $$
  SELECT public.reverse_transaction(p_tx_id, p_kind, p_reason, p_valid_until, false);
$$;
