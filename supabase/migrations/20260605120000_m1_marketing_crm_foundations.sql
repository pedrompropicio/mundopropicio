...
-- ============================================================
-- Helpers genéricos (CREATE OR REPLACE — safe se já existirem)
-- ============================================================
...
CREATE POLICY event_marketing_select ON public.event_marketing
...
CREATE POLICY event_marketing_company_isolation ON public.event_marketing
...
-- ============================================================
-- 2. static_pages (Sobre / Contacto / FAQ unificada)
-- ============================================================
...
CREATE POLICY static_pages_select ON public.static_pages
  FOR SELECT TO authenticated USING (true);
...
CREATE POLICY static_pages_company_isolation ON public.static_pages
  AS RESTRICTIVE FOR ALL TO authenticated
  USING (public.row_belongs_to_current_company(company_id))
  WITH CHECK (public.row_belongs_to_current_company(company_id));
...
-- ============================================================
-- 3. audiences (definições de segmento)
-- ============================================================
...
CREATE POLICY audiences_select ON public.audiences
  FOR SELECT TO authenticated USING (true);
CREATE POLICY audiences_write ON public.audiences
  FOR ALL TO authenticated
  USING (
    public.has_role(auth.uid(), 'admin'::public.app_role) OR
...
CREATE POLICY audiences_company_isolation ON public.audiences
  AS RESTRICTIVE FOR ALL TO authenticated
  USING (public.row_belongs_to_current_company(company_id))
  WITH CHECK (public.row_belongs_to_current_company(company_id));
...
CREATE POLICY audience_snapshots_select ON public.audience_snapshots
  FOR SELECT TO authenticated USING (true);
CREATE POLICY audience_snapshots_write ON public.audience_snapshots
  FOR ALL TO authenticated
  USING (
    public.has_role(auth.uid(), 'admin'::public.app_role) OR
    public.has_role(auth.uid(), 'marketing_manager'::public.app_role)
...
CREATE POLICY audience_snapshots_company_isolation ON public.audience_snapshots
  AS RESTRICTIVE FOR ALL TO authenticated
  USING (public.row_belongs_to_current_company(company_id))
  WITH CHECK (public.row_belongs_to_current_company(company_id));
...
COMMENT ON TABLE public.event_marketing IS 'Conteudo marketing 1:1 com events. Marketing edita; portal le via events_public LEFT JOIN (M2). Workflow draft|published.';
