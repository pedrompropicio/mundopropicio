-- 1) RLS: editor passa a poder UPDATE/DELETE em transaction_payments
DROP POLICY IF EXISTS "Transaction payments updatable by admin or manager" ON public.transaction_payments;
DROP POLICY IF EXISTS "Transaction payments deletable by admin or manager" ON public.transaction_payments;

CREATE POLICY "Transaction payments updatable by privileged roles"
...
CREATE POLICY "Transaction payments deletable by privileged roles"
ON public.transaction_payments
FOR DELETE
TO authenticated
USING (
...
-- 2) RPC: renegociação também para quem tem manage_transactions
