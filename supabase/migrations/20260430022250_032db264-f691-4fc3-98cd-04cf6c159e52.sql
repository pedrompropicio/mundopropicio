-- 1) account_categories: flag de alocação a evento ativo
...
-- 2) events: flags de absorção e janela administrativa
...
-- 3) Validação por trigger (CHECK não pode ser imutável com lógicas cruzadas)
...
AS $$
BEGIN
  -- Splits (com parent_event_id) NUNCA podem absorver
  ...
  IF NEW.admin_window_start IS NOT NULL AND NEW.admin_window_end IS NOT NULL THEN
    IF NEW.admin_window_end < NEW.admin_window_start THEN
      RAISE EXCEPTION 'admin_window_end (%) tem de ser >= admin_window_start (%)',
        NEW.admin_window_end, NEW.admin_window_start;
    END IF;
  END IF;

  -- Se absorve, ambas as datas têm de estar preenchidas
...
-- 4) Validação por trigger em account_categories: flag só em L3 do Group 10
...
BEGIN
  IF NEW.allocate_to_active_event = true THEN
    -- Só categorias cujo código começa com '10' e tem 2 pontos (L3, ex.: 10.7.01)
