-- Recovery migration — trackear em repo as funções process_lead_captures_batch
-- e process_redirect_logs_batch que foram criadas via SQL Editor directo em Live
-- (anti-pattern Sprint 1 portal). Já existem em Live; CREATE OR REPLACE garante
-- idempotência sem impacto. Sem esta migration, próxima perda de schema ou
-- reset Test→Live perde estas funções.
--
-- Definições extraídas de Live (sfohvvlqccmmebvjgibx) em 05/06/26.
...
GRANT EXECUTE ON FUNCTION public.process_lead_captures_batch(integer) TO service_role;
GRANT EXECUTE ON FUNCTION public.process_redirect_logs_batch(integer) TO service_role;
