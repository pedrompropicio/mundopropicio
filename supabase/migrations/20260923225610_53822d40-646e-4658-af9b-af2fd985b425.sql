...
BEGIN
  ...
  IF s.release_date IS NULL THEN
    v_notas := v_notas || to_jsonb('Música sem data de lançamento: todas as bases são primeira medição.'::text);
  END IF;

  -- ===== helper inline: linhas de mesma source (música) =====
  FOR spec IN
    SELECT * FROM (VALUES
  ...
  LOOP
    ...
    IF b.d IS NULL OR a.d IS NULL OR a.d = b.d THEN
      v_notas := v_notas || to_jsonb(format('%s %s: sem 2 medições em datas diferentes — linha omitida.', spec.canal, spec.indicador));
      CONTINUE;
    END IF;
    ...
    IF spec.metric = 'streams' THEN v_streams_val := a.v; v_streams_date := a.d; END IF;
  ...
  -- ===== S4A (manual): base = 1.º registo, atual = mais recente =====
  FOR v_s4a_metric IN
  ...
  LOOP
    ...
    IF a.d = b.d THEN
      v_notas := v_notas || to_jsonb(format('S4A %s: só há registo em %s — linha omitida.', v_s4a_metric, b.d));
      CONTINUE;
    END IF;
    v_pct := CASE WHEN b.v = 0 THEN NULL ELSE (a.v - b.v) / b.v * 100 END;
    v_pct := CASE WHEN v_pct IS NULL THEN NULL WHEN abs(v_pct) < 0.1 THEN round(v_pct, 2) ELSE round(v_pct, 1) END;
  ...
  -- ===== ugc_videos: precedência ios_shortcut > manual; tiktok_artists nunca =====
  WITH pts AS (
  ...
  BEGIN
    ...
    IF vb IS NULL OR va IS NULL OR (vb->>'d') = (va->>'d') THEN
      v_notas := v_notas || to_jsonb('TikTok ugc_videos: sem 2 medições em datas diferentes — linha omitida.'::text);
    ELSE
      ...
      IF (vb->>'src') <> (va->>'src') THEN
        v_notas := v_notas || to_jsonb(format('ugc_videos: base %s (%s) e atual %s (%s) — excepção de precedência ios_shortcut > manual.',
          vb->>'d', vb->>'src', va->>'d', va->>'src'));
      END IF;
    END IF;
  END;

  -- ===== semana de streams (pontos exactos, sem interpolar) =====
  SELECT max(metric_date) INTO v_last FROM public.artist_song_metrics_daily
    WHERE song_id = p_song_id AND platform='spotify' AND metric='streams' AND source='aggregator' AND metric_date <= p_to;
  IF v_last IS NOT NULL THEN
    SELECT value INTO v_c0 FROM public.artist_song_metrics_daily WHERE song_id=p_song_id AND platform='spotify' AND metric='streams' AND source='aggregator' AND metric_date=v_last;
    SELECT value INTO v_c7 FROM public.artist_song_metrics_daily WHERE song_id=p_song_id AND platform='spotify' AND metric='streams' AND source='aggregator' AND metric_date=v_last-7;
    SELECT value INTO v_c14 FROM public.artist_song_metrics_daily WHERE song_id=p_song_id AND platform='spotify' AND metric='streams' AND source='aggregator' AND metric_date=v_last-14;
    ...
    IF v_c7 IS NULL THEN v_notas := v_notas || to_jsonb(format('Streams: falta o ponto de %s — semana atual não calculada.', v_last-7)); END IF;
    IF v_c14 IS NULL THEN v_notas := v_notas || to_jsonb(format('Streams: falta o ponto de %s — semana anterior não calculada.', v_last-14)); END IF;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'series', jsonb_build_object(
      'streams_acumulados', coalesce((SELECT jsonb_agg(jsonb_build_object('d',metric_date,'v',value) ORDER BY metric_date)
          FROM public.artist_song_metrics_daily WHERE song_id=p_song_id AND platform='spotify' AND metric='streams' AND source='aggregator' AND metric_date <= p_to), '[]'::jsonb),
      'ouvintes_mensais', coalesce((SELECT jsonb_agg(jsonb_build_object('d',metric_date,'v',value) ORDER BY metric_date)
          FROM public.artist_metrics_daily WHERE artist_id=s.artist_id AND platform='spotify' AND metric='monthly_listeners' AND source='aggregator'
            AND metric_date <= p_to AND (s.release_date IS NULL OR metric_date >= s.release_date - 7)), '[]'::jsonb)),
    'notas', v_notas,
    'gerado_em', now());
END $$;
