CREATE OR REPLACE FUNCTION public.renegotiate_transaction_installments(
...
-- Esta função NAO recalcula IVA: apenas valida que a soma dos amounts bate com
-- transactions.amount (tolerancia 0.01). O sufixo "(i/N)" na descricao e cosmetico
-- e nunca e lido: a identificacao do grupo e estrutural (installment_group_id).
DECLARE
  v_uid uuid := auth.uid();
...
BEGIN
  IF NOT (public.has_role(v_uid, 'admin'::app_role) OR public.has_role(v_uid, 'manager'::app_role)) THEN
    RAISE EXCEPTION 'permission_denied: apenas administradores ou gestores podem renegociar em parcelas';
  END IF;
...
REVOKE ALL ON FUNCTION public.renegotiate_transaction_installments(uuid, jsonb, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.renegotiate_transaction_installments(uuid, jsonb, text) TO authenticated;