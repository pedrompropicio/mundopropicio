...
BEGIN
  ...
  IF p_kind = 'supplier_credit' THEN
    ...
    UPDATE public.transactions
       SET status = 'reversed', reversal_kind = 'supplier_credit',
           reversal_reason = p_reason, reversed_at = now(),
           reversed_by = v_uid, supplier_credit_id = v_credit_id, updated_at = now()
     WHERE id = p_tx_id;
  ELSE
    UPDATE public.transaction_payments
    ...
    UPDATE public.transactions
       SET status = 'reversed', paid_amount = 0, payment_date = NULL,
           reversal_kind = 'cash_refund', reversal_reason = p_reason,
           reversed_at = now(), reversed_by = v_uid, updated_at = now()
     WHERE id = p_tx_id;
  END IF;

  -- Movimento visível no histórico da transação
  ...
  INSERT INTO public.system_audit_log (
    ...
    jsonb_build_object('status','reversed','reversal_kind',p_kind,
      'reversal_reason',p_reason,'supplier_credit_id',v_credit_id),
    jsonb_build_object('reason',p_reason,'amount',v_paid_amount,
      'supplier_credit_id',v_credit_id,'valid_until',p_valid_until),
    v_tx.company_id
  );

  RETURN jsonb_build_object('ok', true, 'transaction_id', p_tx_id,
    'kind', p_kind, 'supplier_credit_id', v_credit_id);
END;
$$;
