...
BEGIN
  ...
  LOOP
    -- emite os dias já totalmente desfeitos
    WHILE v_day >= v_from AND r.created_at < (v_day + 1)::timestamptz LOOP
      IF v_day <= v_to THEN
        v_series := v_series || public._bp_evolution_day(v_day, v_state);
      END IF;
      v_day := v_day - 1;
    END LOOP;
  ...
  -- dias restantes (sem mais movimentos para desfazer)
...
CREATE OR REPLACE FUNCTION public._bp_evolution_day(_day date, _state jsonb)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $function$
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
           ...
           'l2_code', p2.code,
           'l1_code', p1.code,
           'amount', round(s.amt, 2)
         ) ORDER BY c.code), '[]'::jsonb)
  FROM (
    SELECT k::uuid AS cat, v::numeric AS amt
    FROM jsonb_each_text(_state) AS e(k, v)
    WHERE round(v::numeric, 2) <> 0
  ) s
  JOIN public.account_categories c ON c.id = s.cat
  LEFT JOIN public.account_categories p2 ON p2.id = c.parent_id
  LEFT JOIN public.account_categories p1 ON p1.id = p2.parent_id
$function$;

REVOKE EXECUTE ON FUNCTION public.event_bp_evolution(uuid, date, date) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public._bp_evolution_day(date, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.event_bp_evolution(uuid, date, date) TO authenticated;
GRANT EXECUTE ON FUNCTION public._bp_evolution_day(date, jsonb) TO authenticated;