-- Migration: guard VIP welcome trigger to MP leads only
-- Issue: tg_lead_capture_vip_welcome was firing for ANY lead_capture row with
-- email + consent, including non-MP leads (e.g. Coala).
-- Fix: add company_id guard right after the existing opt-in guard.
