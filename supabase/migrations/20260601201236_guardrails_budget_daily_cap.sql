-- Guardrails de budget — cap diário por role, enforced server-side.
-- Item F do plano Anitta 18/07.
--
-- Adiciona crm.role_budget_limits.max_daily_budget_eur (NULL = sem limite) e a
-- função public.get_user_max_daily_budget_eur(uuid) que devolve o cap mais
-- permissivo entre TODOS os roles do user (multi-role aware), com fail-safe.
--
-- Idempotente: ADD COLUMN IF NOT EXISTS + UPDATE/INSERT ON CONFLICT +
-- CREATE OR REPLACE. Seguro de correr múltiplas vezes.

-- 1) Coluna nova (não toca em monthly_cap_eur nem per_campaign_cap_eur)
...
-- 3) platform_admin (não estava no seed) — sem limite
...
-- 4) Função: cap diário máximo para um user (multi-role aware)
--    NULL  → sem limite (algum role do user tem cap NULL)
--    >0    → MAX(cap) entre os roles do user mapeados em role_budget_limits
--    0     → user sem nenhum role mapeado (fail-safe: bloqueia)
