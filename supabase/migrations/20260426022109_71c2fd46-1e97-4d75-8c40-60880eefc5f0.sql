-- Promote a scenario draft to active version (creates a new active snapshot from it)
...
BEGIN
  ...
  -- Find current active to supersede
  ...
  -- Create new active version from the scenario's snapshot payload
  ...
  -- Unpin the original scenario (kept as draft for history)
  UPDATE public.bp_versions
  ...
  -- Replace event_forecasts with the scenario snapshot (same approach as revert)
