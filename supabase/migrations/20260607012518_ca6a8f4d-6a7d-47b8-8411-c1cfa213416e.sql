...
-- (A) Events da própria company com portal_visible
SELECT 
  e.id, e.slug,
  COALESCE(e.title_pt, e.name) AS title_pt,
  COALESCE(e.title_en, e.name) AS title_en,
  e.description_pt, e.description_en,
  ...
  e.poster_image_url, e.venue_map_url, e.venue_directions_url,
  e.ticketing_url, e.meta_pixel_id,
  e.portal_featured AS featured,
  (e.date < CURRENT_DATE) AS is_past,
  em.hook_pt, em.hook_en,
  em.description_long_pt, em.description_long_en,
  em.meta_description_pt, em.meta_description_en,
  em.og_image_url, em.poster_vertical_url, em.gallery_urls,
  em.press_quote_pt, em.press_quote_en, em.press_quote_source,
  em.cta_primary_label_pt, em.cta_primary_label_en,
  em.urgency_message_pt, em.urgency_message_en,
  em.performer_name, em.performer_url,
  em.offer_price_min, em.offer_price_max, em.offer_currency, em.offer_availability,
...
-- (B) Events endossados cross-company
SELECT 
  e.id, e.slug,
  COALESCE(e.title_pt, e.name) AS title_pt,
  COALESCE(e.title_en, e.name) AS title_en,
