...
begin
  ...
  select count(*), count(distinct l.company_id), min(l.company_id)
...
REVOKE EXECUTE ON FUNCTION public.launch_from_bank_lines(jsonb) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.launch_from_bank_lines(jsonb) FROM anon;
