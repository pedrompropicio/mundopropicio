...
VALUES (
  'rateio_filhas_nao_somam_a_mae',
  'Rateio multi-evento: soma das filhas diferente do valor da mãe (tolerância 0,05 €)',
  'error',
  'empresa',
  0,
  'D-ERP70. A mãe é a fatura inteira; as filhas são a decomposição por evento. Diferença = valor sem evento atribuído, que não aparece no DRE de evento nenhum. Exclui parcelas de pagamento (installment_group_id preenchido). Semeada a 16/09/2026 com 1 caso real a falhar: fatura Meta 252466632 de 26/06/2026, 9.995,23 € contra 8.820,66 € repartidos — faltam 1.174,57 €.'
)
...
REVOKE EXECUTE ON FUNCTION public._run_invariant_checks_raw() FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public._run_invariant_checks_raw() FROM anon, authenticated;
