-- Adenda D-ERP144 (25/09/2026): Resultados TikTok = video_views_6s
-- artist_ads_campaigns: results_30d no ramo tiktok = soma de video_views_6s dos ultimos 30 dias
-- artist_ads_daily: results no ramo tiktok = video_views_6s do dia
-- Mesma regra de nivel: 'campaign' quando existe nesse dia, senao soma dos 'adgroup'.

create temp table _erp144_hash(kind text primary key, h text);

do $$
begin
  perform set_config('request.jwt.claims', json_build_object('sub','d8e502f7-9ceb-4dae-bd73-7291832d0d6f','role','authenticated')::text, true);
  insert into _erp144_hash
  select 'campaigns', md5(string_agg(t::text,'|' order by t::text))
  from (select * from public.artist_ads_campaigns('b1a53be0-e8a8-45c0-bc32-5eea23f477ed', true) where platform in ('meta','google')) t;
  insert into _erp144_hash
  select 'daily', md5(string_agg(t::text,'|' order by t::text))
  from (select * from public.artist_ads_daily('b1a53be0-e8a8-45c0-bc32-5eea23f477ed', 90) where platform in ('meta','google')) t;
end $$;
...
CREATE OR REPLACE FUNCTION public.artist_ads_daily(p_artist_id uuid, p_days integer DEFAULT 90)
...
-- Prova: Meta e Google byte a byte iguais; privilégios mantidos
do $$
declare v_h text; v_ref text;
begin
  perform set_config('request.jwt.claims', json_build_object('sub','d8e502f7-9ceb-4dae-bd73-7291832d0d6f','role','authenticated')::text, true);

  select md5(string_agg(t::text,'|' order by t::text)) into v_h
  from (select * from public.artist_ads_campaigns('b1a53be0-e8a8-45c0-bc32-5eea23f477ed', true) where platform in ('meta','google')) t;
  select hh.h into v_ref from _erp144_hash hh where hh.kind='campaigns';
  if v_h is distinct from v_ref then
    raise exception 'D-ERP144: linhas meta/google de artist_ads_campaigns mudaram'; end if;

  select md5(string_agg(t::text,'|' order by t::text)) into v_h
  from (select * from public.artist_ads_daily('b1a53be0-e8a8-45c0-bc32-5eea23f477ed', 90) where platform in ('meta','google')) t;
  select hh.h into v_ref from _erp144_hash hh where hh.kind='daily';
  if v_h is distinct from v_ref then
    raise exception 'D-ERP144: linhas meta/google de artist_ads_daily mudaram'; end if;

  if has_function_privilege('anon','public.artist_ads_campaigns(uuid,boolean)','EXECUTE') then raise exception 'anon campaigns'; end if;
  if not has_function_privilege('authenticated','public.artist_ads_campaigns(uuid,boolean)','EXECUTE') then raise exception 'authenticated campaigns'; end if;
  if not has_function_privilege('service_role','public.artist_ads_campaigns(uuid,boolean)','EXECUTE') then raise exception 'service_role campaigns'; end if;
  if has_function_privilege('anon','public.artist_ads_daily(uuid,integer)','EXECUTE') then raise exception 'anon daily'; end if;
  if not has_function_privilege('authenticated','public.artist_ads_daily(uuid,integer)','EXECUTE') then raise exception 'authenticated daily'; end if;
  if not has_function_privilege('service_role','public.artist_ads_daily(uuid,integer)','EXECUTE') then raise exception 'service_role daily'; end if;
end $$;

drop table _erp144_hash;