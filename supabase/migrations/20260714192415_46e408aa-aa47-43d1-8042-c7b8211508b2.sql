...
BEGIN
  ...
  -- "Realizado" = todas as despesas lançadas (pagas ou por pagar),
  -- excluindo transações estornadas (reversed_at IS NOT NULL) e escondidas.
  -- UNION direct (event_forecasts.transaction_id) + match por category_id
  -- (parcelas BP — memoria core bp-installments).
  WITH ef AS (
