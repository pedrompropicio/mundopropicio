...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET") {
    ...
    // Verify token: secrets META_WA_* não chegam ao Deno.env das edge functions neste projeto.
    // O fallback é intencional — sem ele o handshake falha. O verify token não é informação sensível (string de handshake público).
