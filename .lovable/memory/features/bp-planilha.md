...
- Colunas: Categoria (read-only) · Descrição · Especificação · Valor s/IVA · Taxa IVA (dropdown
  ...
  (dropdown com empresa + sócios `can_order`) · **Pagador** (dropdown com empresa + sócios
  `can_pay`) · **Anexos** (read-only, contador
  "🔗 N" clicável → painel lateral com as transações vinculadas).
- Filtros **Ordenador** e **Pagador** na toolbar (Todos | Empresa | sócios elegíveis) filtram linhas
  do BP e as transações do painel de anexos (ordenador/pagador efetivo via helpers SSoT).
