...
- `documento_sem_ficheiro_no_storage` (error, global, referência 8, 20/09/2026, issue #213) — ficheiros DISTINTOS (não linhas) em `transaction_documents.file_url` sem objecto correspondente no bucket `transaction-documents`, excluindo os esquemas `bank://`, `ref://`, `camarim://` e `card://`. Único invariante com conformidade por `<=` (a referência só desce quando os 8 ficheiros de Abril/Agosto 2026 forem repostos por upload); qualquer subida significa um caminho de escrita a gravar um `file_url` que não bate com o objecto carregado. Vive em `_run_invariant_checks_docs()`, SECDEF com `EXECUTE` só a `service_role`, unida por `_run_invariant_checks_all()`.
