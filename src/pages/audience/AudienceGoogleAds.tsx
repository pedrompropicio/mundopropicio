// Página Google Ads no MP Audience — conversões offline + publicador de campanhas.
//
...
// selector de período e a moeda da conta.
// Customer Match / Audiences continua no MP CRM (/crm/google-ads).
//
// Publicador (aba "Publicar campanha"): cria campanhas de Pesquisa a partir do ERP,
// sempre EM PAUSA, com pré-visualização e ativação explícita.

import { useState } from "react";
import { Link } from "react-router-dom";
import { useQuery } from "@tanstack/react-query";
import { Activity, AlertTriangle, ArrowRight, KeyRound, MousePointerClick, Rocket } from "lucide-react";
...
import GooglePublishPanel from "@/components/crm/GooglePublishPanel";

export default function AudienceGoogleAds() {
  const [eventId, setEventId] = useState<string>("");

  const { data: eventos } = useQuery({
    queryKey: ["audience-google-events"],
    queryFn: async () => {
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Publicação de campanhas de Pesquisa e conversões offline. O desempenho vive no
      ...
      <Tabs defaultValue="publicar">
        <TabsList>
          <TabsTrigger value="publicar">
            <Rocket className="h-3.5 w-3.5 mr-1.5" /> Publicar campanha
          </TabsTrigger>
          <TabsTrigger value="conversoes">
            <MousePointerClick className="h-3.5 w-3.5 mr-1.5" /> Conversões offline
          </TabsTrigger>
        </TabsList>

        <TabsContent value="publicar" className="space-y-5 pt-4">
          <Card>
            <CardContent className="py-4 space-y-2 max-w-md">
              <Label>Evento</Label>
              <Select value={eventId} onValueChange={setEventId}>
                <SelectTrigger>
                  <SelectValue placeholder="Escolhe o evento a promover" />
                </SelectTrigger>
                <SelectContent>
                  {(eventos ?? []).map((e: any) => (
              ...
              <p className="text-xs text-muted-foreground">
                A campanha nasce sempre em pausa: nada gasta até a ativares explicitamente.
              </p>
            </CardContent>
          </Card>

          {eventId ? (
            <GooglePublishPanel eventId={eventId} />
          ) : (
            <p className="text-sm text-muted-foreground">Escolhe um evento para começar.</p>
          )}
        </TabsContent>

        <TabsContent value="conversoes" className="space-y-5 pt-4">
          <Card>
