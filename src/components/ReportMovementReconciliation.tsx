...
export default function ReportMovementReconciliation() {
  ...
  const { data: allTransactions = [] } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        .from("transactions")
        .select("*, events(name, parent_event_id), suppliers:suppliers!transactions_supplier_id_fkey(name), financial_accounts:financial_accounts!transactions_account_id_fkey(name)")
