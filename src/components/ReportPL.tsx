...
import { ChevronDown, ChevronRight, FileText, Download, TrendingUp, TrendingDown, BarChart3 } from "lucide-react";
import { Checkbox } from "@/components/ui/checkbox";
import { exportPLToPDF, exportPLToExcel } from "@/lib/export-pl";

function calcWithIva(amount: number, ivaRate: number) {
...
function buildPL(forecasts: any[], transactions: any[], categories: any[]): PLLine[] {
  const catMap = Object.fromEntries(categories.map((c: any) => [c.id, c.name]));

  const aggregate = (items: any[], field: "amount") => {
    const byCat: Record<string, number> = {};
    items.forEach((item) => {
      const name = catMap[item.category_id] ?? "Sem categoria";
      byCat[name] = (byCat[name] ?? 0) + Number(item[field]);
    });
    return byCat;
  };
  ...
  const fIncByCat = aggregate(fInc, "amount");
  const fExpByCat = aggregate(fExp, "amount");
  const tIncByCat = aggregate(tInc, "amount");
  const tExpByCat = aggregate(tExp, "amount");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-end gap-2">
        <button
          onClick={() => exportPLToPDF(activeEvents, forecasts, transactions, categories)}
        ...
        <button
          onClick={() => exportPLToExcel(activeEvents, forecasts, transactions, categories)}
          disabled={activeEvents.length === 0}
      ...
      <div className="space-y-3">
        {eventSummaries.map((evt) => {
          ...
          const pl = isOpen ? buildPL(evtF, evtT, categories) : [];
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {pl.map((line, i) => {
                          ...
                          const valClass = (isVariance?: boolean) =>
                            `text-right font-mono ${line.isGrandTotal ? "text-base font-bold" : line.isTotal ? "font-semibold" : "text-muted-foreground"}`;
                          const varianceColor = (v: number, isExpense: boolean) => {
                            if (v === 0) return "";
                            // For expenses, negative variance (spent less) is good
                            // For revenues/result, positive variance is good
                            if (line.label === "DESPESAS" || (line.indent && i > pl.findIndex((l) => l.label === "DESPESAS"))) {
                          ...
                          return (
                            <TableRow key={i} className={rowClass}>
                              ...
                              <TableCell className={valClass()}>{formatCurrency(Math.abs(line.forecast))}</TableCell>
                              <TableCell className={valClass()}>{formatCurrency(Math.abs(line.actual))}</TableCell>
                              <TableCell className={`text-right font-mono ${line.isGrandTotal ? "text-base font-bold" : line.isTotal ? "font-semibold" : ""} ${varianceColor(line.variance, false)}`}>
