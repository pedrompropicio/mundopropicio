...
export function TransactionEditModal({ transaction, onClose, canApprove }: Props) {
  ...
  // Herdado da conta: contas gerenciais (is_accounting=false) não exportam para a contabilidade
  const { data: txAccount } = useQuery({
    queryKey: ["transaction-account-accounting", transaction.account_id],
  ...
  const accountNotAccounting = txAccount?.is_accounting === false;
  // Conta restrita → is_confidential é forçado pelo trigger (D-ERP34 ponto 5)
  const confidentialForcedByAccount = Boolean(
    (financialAccounts as any[]).find((a) => a.id === (form?.account_id || transaction.account_id))?.is_restricted
      ?? (txAccount as any)?.is_restricted
  );
  ...
  const editMutation = useMutation({
    mutationFn: async () => {
      ...
      const fieldLabels: Record<string, string> = {
        ...
        shared_cost_account_id: "Conta de rateio de terceiros",
        shared_cost_counterparty_id: "Terceiro (rateio)",
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4">
      <div className="glass w-full max-w-lg rounded-xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        ...
        {accountNotAccounting && (
          <div className="inline-flex items-center gap-1.5 rounded-lg bg-secondary px-3 py-1.5 text-xs font-medium text-muted-foreground">
            Conta não contábil
            <HelpTooltip text="A conta financeira desta transação está marcada como não contábil: os seus documentos não são exportados para a contabilidade." />
          </div>
        )}
        ...
        <form onKeyDown={blockImplicitSubmitOnEnter} onSubmit={handleSubmit} className="space-y-4">
          ...
          <OperationKeySelector
            ...
            onChange={(v) => setForm({ ...form, operation_key: v })}
          />
