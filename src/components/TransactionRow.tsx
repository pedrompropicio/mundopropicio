...
export function TransactionRow({ transaction: t, isAdmin, selectable, selected, onToggleSelect, showSelectColumn, eventCompleted, showPaymentDate, onEdit, onApprove, onPayment, onDocs, onAudit, onDelete, onToggleHidden, onViewPayments, highlightId, inGroup, hasInstallments }: Props) {
  ...
  return (
    <>
      <tr ref={rowRef} className={`border-l-2 border-l-transparent hover:bg-secondary/20 transition-colors ${computedStatus === "paid" ? "opacity-80" : ""} ${selected ? "bg-primary/5" : ""} ${isHidden ? "opacity-50 bg-muted/20" : ""} ${isHighlighted ? "ring-2 ring-primary ring-inset bg-primary/10 animate-pulse" : ""} ${inGroup ? "border-l-primary bg-primary/5" : ""}`}>
        {showSelectColumn && (
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                {t.is_transitory && (
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <span className={`inline-flex items-center gap-0.5 rounded border px-1.5 py-0.5 text-[10px] font-semibold cursor-help ${
                        isPartnerExtra
                          ? "border-warning/40 bg-warning/10 text-warning"
                          : "border-purple-500/30 bg-purple-500/10 text-purple-600 dark:text-purple-400"
                      }`}>
                        {isPartnerExtra ? "🧳 Extra Sócio" : "🔄 Transitória"}
                      </span>
                    </TooltipTrigger>
                    <TooltipContent side="top" className="max-w-xs text-xs">
                      <p>{isPartnerExtra
                        ? "Despesa paga pela empresa, descontada do sócio no fecho do evento. Não impacta DRE/BP."
                        : "Transação transitória (caução/depósito) — não impacta o resultado do evento."}</p>
                    </TooltipContent>
                  </Tooltip>
                )}
        ...
        <td className={`py-2 pr-2 text-right whitespace-nowrap ${isExpense ? "text-warning" : "text-success"}`}>
          ...
          {isExpense && (() => {
            ...
            const label = isPaid ? "Líquido pago" : "A pagar";
            return (
              <Tooltip>
                <TooltipTrigger asChild>
                  <p className="mt-0.5 text-[10px] font-mono text-warning cursor-help">
                    {label}: <span className="font-semibold">{formatCurrency(np.net)}</span>
                ...
                <TooltipContent side="top" className="max-w-xs text-xs">
                  ...
                  <p className="mt-1 text-muted-foreground">
                    O fornecedor {isPaid ? "recebeu" : "recebe"} o líquido; a retenção é entregue ao Estado.
                    {isPaid ? " Valor bruto mantido para efeitos de BP/DRE." : " Valor pode ser ajustado no momento da liquidação."}
                  </p>
                </TooltipContent>
              </Tooltip>
            );
