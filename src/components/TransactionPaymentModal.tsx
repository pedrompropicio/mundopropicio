    ...
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
        <div className="glass w-full max-w-sm rounded-xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
          ...
          {isForeign && (() => {
            // Original (foreign) amount contracted — fixed reference
            const origAmountFx = Number(transaction.original_amount) || 0;
            const origRate = Number(transaction.fx_rate) || 0;
            // Outstanding portion of the foreign amount (proportional to EUR balance)
            const totalEur = amount;
            const balanceFx = totalEur > 0 ? +(origAmountFx * (balance / totalEur)).toFixed(2) : origAmountFx;
            return (
              <div className="rounded-lg border border-border/60 bg-muted/30 p-3 space-y-2">
                <div className="flex justify-between text-[11px] text-muted-foreground">
                  <span>Valor contratado em {txCurrency}:</span>
                  <span className="font-semibold text-foreground">{formatInCurrency(balanceFx, txCurrency)}</span>
                </div>
                  ...
                  <div className="flex-1">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (r > 0 && balanceFx > 0) {
                          setPaymentAmount((balanceFx * r).toFixed(2));
                        }
                      }}
                      className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50"
                      placeholder={origRate ? String(origRate) : "Ex.: 0.17"}
                    />
                  </div>
                  <button
                    type="button"
                    onClick={async () => {
                      ...
                      setLoadingFx(false);
                      if (r) {
                        setPaymentFxRate(r.toFixed(6));
                        if (balanceFx > 0) setPaymentAmount((balanceFx * r).toFixed(2));
                      }
                    }}
                    disabled={loadingFx}
                ...
                {(() => {
                  const rate = parseFloat(paymentFxRate) || 0;
                  if (rate <= 0 || balanceFx <= 0) return (
                    <p className="text-[11px] text-muted-foreground">
                      Define o câmbio do dia para recalcular o valor em EUR a pagar.
                    </p>
                  );
                  const eurNow = +(balanceFx * rate).toFixed(2);
                  const eurOriginal = +(balanceFx * origRate).toFixed(2);
                  const diffEur = origRate > 0 ? +(eurNow - eurOriginal).toFixed(2) : 0;
                  const diffRate = origRate > 0 ? rate - origRate : 0;
                  return (
                    <div className="text-[11px] text-muted-foreground space-y-0.5">
                      <p>
                        {formatInCurrency(balanceFx, txCurrency)} × {rate.toFixed(6)} ={" "}
                        <span className="font-semibold text-foreground">{formatCurrency(eurNow)}</span>
                      </p>
                      {origRate > 0 && (
                        <p className={diffEur >= 0 ? "text-warning" : "text-success"}>
                          vs. câmbio original ({origRate.toFixed(6)} → {formatCurrency(eurOriginal)}):{" "}
                          {diffEur >= 0 ? "+" : ""}{formatCurrency(diffEur)}{" "}
                          ({diffRate >= 0 ? "+" : ""}{diffRate.toFixed(6)} €/{txCurrency})
                        </p>
                      )}
                    </div>
                  );
