...
function buildDREBrasil(
  ...
  // Overheads (BP) — vista do sócio. Alocados DENTRO da categoria respetiva
  ...
  if (eventPartners.length > 0) {
    ...
    // residual `100 − Σ(externos do evento)`. Usar `eventPartners` (deste evento)
    // e não `partners` (lista global de todos os eventos).
...
export default function ReportDREBrasil() {
  ...
  // Distribuição global aos sócios (excluindo a Mundo Propício, que é a sócia
  // principal/proprietária do sistema e tem o seu próprio agregado).
    ...
    <div className="space-y-6">
      ...
      <div className="glass rounded-xl p-4 flex items-center gap-3">
        ...
        <span className="text-xs text-muted-foreground ml-2">
          {showPartnerView ? "Inclui rateios de overhead e extras no resultado" : "Vista Empresa: sem overhead"}
        </span>
      </div>
