...
const META: Record<string, { label: string; cls: string }> = {
  ...
  setup: { label: "Setup", cls: "bg-muted text-muted-foreground border-border" },
};
...
export function PhaseBadge({ phase, className }: { phase: OperacaoPhase; className?: string }) {
  ...
  return (
    <Badge
      variant="outline"
      className={cn(
        "h-6 px-2 text-[10px] font-semibold border uppercase tracking-wide whitespace-nowrap shrink-0 leading-none inline-flex items-center",
        m.cls,
        className,
      )}
    >
