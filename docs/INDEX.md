...
**Lotes e preços no BI de Vendas (#214):** RPC única `public.get_event_zone_price_dynamics(uuid)` (jsonb, SECURITY DEFINER, `anon` = false, `authenticated` + `service_role` = true, isolamento por `row_belongs_to_current_company`) alimenta o separador "Lotes e preços" de `/vendas/:groupId/:eventId`. Ocupação é sempre sobre o LIBERTADO (`event_zone_capacities`, `capacity_kind='released'`) e nunca se lê um setof grande de `ticket_sales` no cliente — ver `.lovable/memory/features/bi-vendas-lotes-precos.md`.
