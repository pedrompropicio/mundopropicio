...
A3-bis: `tiktok-artists-sync` passa a chamar também `ttfa/clip_data_list/` por cada `group_id` mapeado e faz upsert dos sons `pgc_*`/`ugc_*` em `artist_song_tiktok_sounds` com `discovered_via='panel'`, `status='validated'`, `is_official` = som `pgc` e `title = clip_name`, sem apagar nada — a descoberta dos sons oficiais do nosso artista fica automática.
...
**Adenda 24/09/2026 — sync YouTube (`artist-youtube-sync`).** verify_jwt=true; service_role ou admin/platform_admin/manager/marketing_manager da empresa do artista (user_roles). Body `{ artist_id?, dry_run? }`, dry_run por omissão true; sem artist_id corre todas as ligações google `active`. Token via `artist_get_connection_token`; se expira em <5 min renova em `oauth2.googleapis.com/token` (refresh_token) e grava com `artist_channel_store_rotated_tokens` (sem refresh novo da Google, repassa o actual); `invalid_grant` → status `expired`, last_error "YouTube: autorização revogada ou expirada — voltar a ligar". Pedidos: (a) `youtube/v3/channels?part=statistics,snippet&mine=true` — tem de conter o external_id, senão erro sem gravar; (b) `youtubeanalytics v2 reports` ids=channel==MINE, D−27..D (D = ontem UTC), dimensions=day. Grava em `artist_metrics_daily` (platform `youtube`, source `platform_api`, upsert pela UNIQUE): (a) metric_date = hoje: `subscribers` (não grava se hiddenSubscriberCount), `video_count`, `views_total`; (b) metric_date = dia: `yt_views_day`, `yt_minutes_watched_day`, `yt_subscribers_gained_day`, `yt_subscribers_lost_day` (zeros gravam-se). Regista em sync_runs. O callback OAuth dispara o sync (dry_run:false) best-effort via `_shared/internal-call.ts` sem bloquear o redirect. Cron diário por criar (Pedro). **Adenda 24/09/2026 (2):** numa resposta 200 do Analytics, os dias ausentes das rows (dias sem actividade, que o Analytics omite) são gravados a 0 nas quatro métricas `yt_*_day`; só com não-200 é que não se gravam zeros.
...
**Adenda 24/09/2026 — semente por código de uso único.** A semente deixa de exigir que outra pessoa faça um pedido com o JWT do Pedro (um refresh_token do Pedro num pedido de terceiros é inaceitável). Fluxo: um admin da empresa do artista gera na app um código (`artist_s4a_seed_code_create`, 10 min, uso único, gerar um novo invalida os anteriores; só o sha256 fica em `public.artist_channel_seed_codes`); no separador do S4A, uma linha de consola lê o refresh_token da própria página e faz POST à `s4a-token-seed` com `{ seed_code, client_id, refresh_token }` e a chave anon pública. A função valida formatos, consome o código com `artist_s4a_seed_code_consume` (service_role only; 0 linhas → 403 `codigo_invalido_ou_expirado`), tira o artist_id do código (nunca do corpo) e usa `created_by` como `connected_by`; o código fica consumido mesmo que a troca falhe. Chave de serviço recusada nesse caminho; máx. 5 tentativas falhadas por IP em 10 min (429, memória da instância). CORS ecoa só `https://artists.spotify.com` e as origens de `allowedReturnOrigins()`. O caminho antigo `{ artist_id, … }` com sessão de admin mantém-se. Privilégios: tabela sem acesso para anon/authenticated; create — authenticated+service_role; consume — só service_role.
...
### Adenda D-ERP95 — vídeos do Instagram como anúncio (26/09/2026)

`crm-meta-publish-execute`, só alvo música (eventos inalterados; dry_run 93529702… = md5 `0e2801d625781a22a1e4bb33fb0a0f6d`). Origem: plano fadb7b06 falhou em create_ad com 100/1815279 ("carregar o vídeo no Facebook"). Posts do Instagram (`instagram_media`) passam a usar o formato documentado v22+ `object_id` (Página) + `source_instagram_media_id` + `instagram_user_id`, em Graph `v25.0` (instagram_user_id substituiu instagram_actor_id na v22); v18.0 mantém-se nos restantes caminhos. Validado com `execution_options=["validate_only"]`: para os vídeos 18194624299387532 e 18622548235024177 a Meta devolve 1815279 em v18 e em v25, com e sem CTA — não há versão que aceite estes vídeos directamente sem upload ao Facebook. Alternativa automática: com 1815279 procura o mesmo conteúdo na Página (mesma legenda normalizada, ±48 h, correspondência única, token da Página só em memória) e tenta `object_story_id`, validando primeiro; se não houver ou não for aceite, recusa com mensagem legível. Em objectivo TRAFFIC o post da Página (sem link) é recusado pela Meta com 1815520. O preflight passa a ler `boost_eligibility_info` e, quando o conjunto já existe (retoma), valida cada anúncio de post Instagram por validate_only — em plano novo sem conjunto não há onde validar (limitação). A retoma reutiliza campaign/adset/ad persistidos; `falhado` já é estado aceite.
...
**Precedência.** Uma linha `api` substitui sempre a `manual` com a mesma chave; uma `manual` NUNCA substitui uma `api` (a RPC salta-as e conta-as em `ignorados_api`).

**Segurança.** RLS ligada; `authenticated` só SELECT pela empresa (`company_id = current_company_id()`), escrita só service_role/RPC; anon nada. RPC SECURITY DEFINER, search_path fixo, exige sessão e papel admin/platform_admin/manager/marketing_manager na empresa da ligação por user_roles (D-ERP128); ligação tem de ser `tiktok`/`artist`. anon false, authenticated true, service_role true.

**Contrato do payload.** `{ campanha: {external_campaign_id?, name, status, objective, budget_cents, song_id?}, grupos: [{external_adgroup_id, name, status, budget_cents}], dias: [{date, level:'adgroup'|'campaign', external_id, spend (moeda da conta, ex. 223.55), impressions, clicks, video_views_6s, video_views_2s, likes, follows, reach}] }`. Id da campanha: explícito > campanha `api` com o mesmo nome > sintético `manual:<slug do nome>` (`tiktok_manual_slug`). `song_id` grava `linked_song_id` + `linked_song_locked=true` (também em linhas api). Números ≥ 0, datas não futuras; `spend` → `spend_cents = round(spend*100)`; moeda = a da ligação. Devolve contagens (campanha/grupos/dias inseridos, atualizados, ignorados_api).

**Painel.** `artist_ads_campaigns` e `artist_ads_daily` ganham o ramo `tiktok` (nível campaign nesse dia se existir, senão soma dos adgroup; vídeo = video_views_6s, reserva 2s; results = 0; `*_ref` por fx_convert ao dia) e duas colunas NO FIM: `data_source` (`api|manual|mixed`; Meta/Google sempre `api`) e `last_recorded_at`. Assinaturas e grants iguais; resultados Meta/Google idênticos antes/depois (hash das linhas igual).
...
### Adenda D-ERP144 — gravação pela tarefa agendada, sem sessão (25/09/2026)

A lógica de gravação passou para `crm.tiktok_manual_upsert_core(p_connection_id, p_payload, p_actor)` (SECURITY DEFINER, search_path fixo, só service_role; valida apenas ligação `tiktok`/`artist`; grava `raw.gravado_por = p_actor` nas linhas de dias). `public.artist_ads_tiktok_manual_upsert` mantém assinatura, grants e verificação de sessão+papel, e chama o core com `auth.uid()`; equivalência provada numa transação anulada (mesmo JSON antes/depois). Porta de ingestão `artist-ads-tiktok-manual-ingest` (verify_jwt=false, só POST): cabeçalho `x-ingest-key` comparado em tempo constante com o secret dedicado `TIKTOK_MANUAL_INGEST_KEY` (sem secret → 503; errada → 401), lista fixa de ligações permitidas (só Litto 947ee0c7-…; outra → 403), 30 pedidos/hora em memória, `p_actor='cowork-ingest'`, cada chamada em `sync_runs` (nunca a chave). Não se usa o token do utilizador: extrair a sessão do Pedro do browser daria à tarefa todos os poderes dele e expiraria; uma chave dedicada só abre esta porta e revoga-se sozinha.

### Adenda D-ERP95 — vídeo do Instagram carregado no Facebook (opção B, 26/09/2026)

- Decisão do Pedro (26/09): quando um anúncio `existing_post` `instagram_media` dá 100/1815279 (validate_only ou criação), o motor (só alvo música, Graph v25.0) lê o media (`media_type,media_url,thumbnail_url,caption,permalink`), faz `POST /{act}/advideos` com `file_url=media_url` e `name="[MP] <título-base> — IG <media_id>"`, sonda `/{video_id}?fields=status` a cada 5 s até `video_status='ready'` (máx. 3 min; se expirar, falha legível e o video_id fica guardado) e cria o criativo `object_story_spec {page_id, instagram_user_id, video_data {video_id, image_url=thumbnail_url, message=caption[:2000], call_to_action {type: LISTEN_NOW em TRAFFIC / LEARN_MORE nos outros, value.link = destino}}}` + `url_tags` do motor.
- Cache: `adsets[i].anuncios[k].meta_video_id` + `origem_ig_media_id`, gravados em todos os anúncios do plano com o mesmo media_id; retomas e outros conjuntos reutilizam sem novo upload.
- Post cruzado da Página: retirado em TRAFFIC (1815520 sempre); mantido nos outros objetivos.
- Preflight/dry_run: aviso "vai ser carregado no Facebook como vídeo do anúncio (perde gostos/comentários do post original)" + GET de 1 KB ao media_url; nunca carrega. Com conjunto existente valida com validate_only usando o video_id já carregado, se houver.
- Registo: `crm.meta_entity_actions_log.entity_type` tem CHECK (campaign/adset/ad) — sem DDL não se pode gravar entity 'video'; o video_id vai no `updates_jsonb` do log do anúncio e em `videos_carregados` no resumo/erro da publicação.
- Evento intacto: dry_run 93529702-… md5 `0e2801d625781a22a1e4bb33fb0a0f6d` (26/09).

Adenda D-ERP95 (26/09, opção A — música licenciada): em `crm-meta-publish-execute` (só alvo música, Graph v25.0), quando o criativo `video_data` com o vídeo da Página equivalente recebe 100/2446979 ("reel com música licenciada, não pode ser turbinado"), o motor lê `GET /{video_id}?fields=source,length,picture` com o token da Página (só em memória), carrega o ficheiro em `/{ad_account}/advideos` (`file_url = source`, nome "[MP] <título> — Page video <id>"), espera `video_status='ready'` (5 s, até 3 min) e cria o criativo limpo com o NOVO video_id. Cache/persistência por vídeo ORIGINAL da Página: `anuncios[k].origem_page_video_id` + `meta_video_id` (+ `origem_ig_media_id`); retomas e outros conjuntos reutilizam sem novo download. Ordem para vídeo IG com 1815279: cache → upload do media_url IG → vídeo da Página → download+upload do source (2446979) → erro legível. Sem `source` → erro legível, vídeo original fica registado. Preflight/dry_run não carregam nada: dizem a via e confirmam que `source` existe; o criativo final só é validado pela Meta depois do upload (limitação). Registo em `videos_carregados` do resumo (`media_id = page_video:<id>`), porque o CHECK de `crm.meta_entity_actions_log.entity_type` não aceita 'video'. Eventos inalterados: md5 0e2801d625781a22a1e4bb33fb0a0f6d confirmado a 26/09.
