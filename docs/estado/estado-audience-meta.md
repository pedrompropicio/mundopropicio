...
## Em que pé está
- **Várias contas Meta por artista (24/09, D-ERP143).** Uma ligação por conta de anúncios, token partilhado e propagado no OAuth. Litto: CA 1 act_323668247351618 (leitura, gestor externo) + conta [MP] act_1818321225966067 (EUR) a acrescentar pelo Pedro via `artist-ads-meta-add-account`; teto definido pelo Pedro.
- **Ligação Meta de empresa vê contas de todos os BMs (23/09, #250).** `crm-meta-fetch-ad-accounts` deixou de filtrar pelo BM guardado e pagina; escolher a conta grava o BM dessa conta em `external_business_id`. Função deployada; ecrã à espera de Publish. Ligações existentes (Mundo Propício, Litto Lins) intactas.
