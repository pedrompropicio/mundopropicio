...
**O saldo fecha em zero porque conta as transferências.** A transferência que o fecho cria é `type='transfer'`; enquanto os ecrãs só contavam `expense`, o saldo nunca descia após um fecho. Não é preciso a fórmula ler `ticket_office_settlements` — basta contar as transferências. Os fechos só são lidos para o indicador de retenção.
