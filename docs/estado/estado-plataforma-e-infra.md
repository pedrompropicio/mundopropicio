...
**A empresa activa só muda pelo seletor e cada mudança fica registada (23/09, D-ERP138).** Um trigger em `profiles` recusa mudanças directas a `active_company_id` feitas com sessão (erro 42501); só `set_active_company` passa. Cada mudança grava em `system_audit_log` (action 'active_company_changed') a empresa antiga e nova, a via, origin, referer e user_agent. Grep em src/ e edge functions: nenhum UPDATE/upsert directo ao campo.
**Folha de crescimento desde o lançamento passa a sair da plataforma (23/09, D-ERP136).** Nova leitura `song_growth_summary` por música: base no lançamento, atual até à data pedida, semana de streams sem interpolar, UGC com precedência do Atalho iOS. Prova contra a folha manual da Roupa de Solteira bate em tudo excepto ugc_videos a 23/09 (sistema 6.700 do Atalho vs 6.600 na folha). Ainda sem ecrã.
**Ligações de artista: anúncios pedem instagram_basic e o YouTube já tem ligação própria; a cadeia de token do Spotify for Artists já está no servidor (23/09, D-ERP133/D-ERP134/D-ERP135).** A ligação de anúncios do artista pede agora também a permissão básica do Instagram e, se a ligação já tiver página escolhida, grava a conta de Instagram dessa página (se não houver, fica registado o motivo e a ligação segue). Nasceu a ligação do canal de YouTube do artista por conta Google: só grava se a conta Google for dona do canal registado e exige renovação offline; desde 24/09 existe a recolha `artist-youtube-sync` (subscritores, vídeos, visualizações totais e série diária de 28 dias do Analytics), disparada logo após ligar; cron diário por criar (adenda D-ERP134). Desde 24/09 o redirect_uri do OAuth YouTube é `https://www.mundopropicio.com/oauth/google/callback` (302 no servidor para a edge function; motivo: verificação da app pela Google, que exige domínios verificados no Search Console — adenda D-ERP134). O Spotify for Artists tem agora a cadeia de token no servidor (D-ERP135): semente por admin, renovação com trinco e gravação imediata do novo par, sonda de leitura; recolha diária ainda por fazer. Desde 24/09 a semente pode ser feita com um código de uso único gerado na app (10 min) e colado numa linha de consola no separador do S4A — ninguém vê o token (adenda D-ERP135).
