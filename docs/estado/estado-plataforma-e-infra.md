...
## Em que pé está
**Ligações de artista: anúncios pedem instagram_basic e o YouTube já tem ligação própria; a cadeia de token do Spotify for Artists ficou bloqueada na base (23/09, D-ERP133/D-ERP134).** A ligação de anúncios do artista pede agora também a permissão básica do Instagram e, se a ligação já tiver página escolhida, grava a conta de Instagram dessa página (se não houver, fica registado o motivo e a ligação segue). Nasceu a ligação do canal de YouTube do artista por conta Google: só grava se a conta Google for dona do canal registado e exige renovação offline; ainda sem recolha de dados e à espera dos segredos do cliente OAuth. A parte do Spotify for Artists não avançou: a migração pedida falhou (a função de trinco escrita como SQL com UPDATE dentro de subconsulta não é aceite pelo Postgres) e nada foi aplicado.
